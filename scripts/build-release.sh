...
rsync -a --exclude .git --exclude node_modules --exclude dist --exclude dist-release --exclude .env --exclude '.zonoe-baota-installed' --exclude '.zonoe-native-installed' --exclude 'data/uploads/*' --exclude 'data/update-runtime/*' --exclude 'data/control/*' --exclude 'backups/*' "$ROOT/" "$TMP/src/"
...
(cd "$TMP/src" && zip -qr "$OUT/$NATIVE_ZIP" . -x '.github/*' 'dist-release/*' 'data/uploads/*' 'data/update-runtime/*' 'data/control/*' 'backups/*' '.env' '.zonoe-baota-installed' '.zonoe-native-installed' 'docker-compose.yml' 'docker-compose.dev.yml' 'apps/api/Dockerfile' 'apps/web/Dockerfile' 'apps/web/nginx.conf' 'deploy/*')
