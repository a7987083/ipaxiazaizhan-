...
function ReadOnlyApps({items}){return <div className="admin-panel"><p>这里是多个 MySQL 软件源的只读聚合视图。新增/修改 App 继续在原软件源后台完成；ZONOE 不会重复上传 IPA。</p><div className="admin-list">{items.map(a=><div className="admin-app" key={a.id}><img src={a.icon_url||'/placeholder.svg'}/><div><b>{a.name}</b><small>{a.source_name} · 原 ID {a.legacy_id}</small><small>v{a.version||'—'} · {Number(a.file_size||0)?`${(Number(a.file_size)/1024/1024).toFixed(1)} MB`:'大小未知'}</small></div><a href={`/app/${encodeURIComponent(a.id)}`} target="_blank" rel="noreferrer">查看</a></div>)}</div></div>}

const blankSource={name:'',slug:'',host:'127.0.0.1',port:3306,database:'',username:'',password:'',table:'fa_category',priority:100,enabled:true,writeStats:false};
function SourceManager({items,refresh}){
  ...
  const edit=x=>{setEditing(x.id);setF({name:x.name,slug:x.slug,host:x.host,port:x.port,database:x.database,username:x.username,password:'',table:x.table||'fa_category',priority:x.priority,enabled:x.enabled,writeStats:x.writeStats})};
  const save=async()=>{setNote('');const body={...f,port:Number(f.port),priority:Number(f.priority),enabled:!!f.enabled,writeStats:!!f.writeStats};if(editing&&!body.password)delete body.password;try{if(editing)await api.admin(`/sources/${editing}`,{method:'PUT',body:JSON.stringify(body)});else await api.admin('/sources',{method:'POST',body:JSON.stringify(body)});reset();await refresh()}catch(e){setNote(e.message)}};
