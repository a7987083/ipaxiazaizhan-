...
import AdminReplicaRepairPanel from './AdminReplicaRepairPanel';
...
export default function Admin(){
  ...
  useEffect(refresh,[]);
  const title={dashboard:'工作台',apps:'应用聚合',sources:'MySQL 软件源',openlist:'IPA 元数据',replicas:'云盘副本',replicarepair:'异常修复',replicaops:'副本任务',writeback:'数据同步',settings:'站点设置',cache:'本地缓存',security:'修改密码',update:'在线更新'}[tab]||'后台';
  const tabs=[['dashboard','工作台'],['apps','应用'],['sources','软件源'],['openlist','IPA 元数据'],['replicas','云盘副本'],['replicarepair','异常修复'],['replicaops','副本任务'],['writeback','数据同步'],['settings','站点设置'],['cache','本地缓存'],['security','密码'],['update','在线更新']];
  ...
  {tab==='replicarepair'&&<AdminReplicaRepairPanel/>}
