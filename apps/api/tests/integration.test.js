...
describe('ZONOE 1210 MySQL catalog-only aggregation',()=>{
  ...
  test('detail and versions read legacy fa_category mapping without download address',async()=>{let r=await request(app).get('/api/v1/apps/source-a:1');expect(r.status).toBe(200);expect(r.body.data.name).toBe('Alpha Game');expect(r.body.data.version).toBe('100');expect(r.body.data.file_size).toBe(104857600);expect(r.body.data.download_url).toBeUndefined();expect(r.body.data._downloadTarget).toBeUndefined();r=await request(app).get('/api/v1/apps/source-a:1/versions');expect(r.body.data[0].version).toBe('100')});
  test('public download endpoint is disabled',async()=>{const r=await request(app).get('/download/source-a:1').redirects(0);expect(r.status).toBe(404)});
  ...
  test('login rate-limit is JSON',async()=>{for(let i=0;i<20;i++)await request(app).post('/api/v1/admin/login').send({username:'missing-rate-limit-user',password:'wrong-password'});const r=await request(app).post('/api/v1/admin/login').send({username:'missing-rate-limit-user',password:'wrong-password'});expect(r.status).toBe(429);expect(r.body.error.code).toBe('LOGIN_RATE_LIMITED');expect(r.body.error.message).toMatch(/登录尝试过多/)});
});
