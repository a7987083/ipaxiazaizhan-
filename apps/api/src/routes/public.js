...
r.get('/apps',asyncHandler(async(req,res)=>{ const result=await listApps(req.query); ok(res,result.items,{page:result.page,pageSize:result.pageSize,total:result.total,totalPages:Math.max(1,Math.ceil(result.total/result.pageSize)),unavailableSources:(result.sourceErrors||[]).map(x=>x.source)}); }));
r.get('/search',asyncHandler(async(req,res)=>{ const result=await listApps({...req.query,q:req.query.q||''}); ok(res,result.items,{page:result.page,pageSize:result.pageSize,total:result.total,totalPages:Math.max(1,Math.ceil(result.total/result.pageSize)),unavailableSources:(result.sourceErrors||[]).map(x=>x.source)}); }));
